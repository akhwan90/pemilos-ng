    ...
    <div class="space-y-6">
        <div class="flex items-center justify-between mb-4">
            <div>
                ...
                <p class="text-sm text-gray-500">
                    Kelola data pemilih (siswa) di sekolah Anda.
                </p>
            </div>
                ...
                <div class="flex gap-2 items-center">
                    ...
                    <div class="relative w-40">
                        <select
                            v-model="filterKelas"
                            @change="fetchSiswa(1)"
                            class="w-full pl-3 pr-8 py-2 border border-gray-300 rounded-lg text-sm focus:ring-indigo-500 focus:border-indigo-500 appearance-none bg-white"
                        >
                            <option value="">Semua Kelas</option>
                            <option
                                v-for="kls in daftarKelas"
                                :key="kls"
                                :value="kls"
                            >
                                {{ kls }}
                            ...
                            <td class="px-4 py-3 text-center">
                                <div class="flex gap-2 justify-center">
                                    <button
                                        ...
                                        class="text-indigo-600 hover:text-indigo-900 bg-indigo-50 px-2 py-1 rounded text-xs"
                                    >
                                        Edit
                                    </button>
                                    <button
                                        @click="openSingleDeleteModal(item.id)"
        ...
        >
            <form @submit.prevent="submitForm" class="space-y-4">
                ...
                <div>
                    <label class="block text-sm font-medium text-gray-700 mb-1"
                        >Status Difabel</label
                    >
                    <select
                        v-model="form.difabel"
                        required
                        class="w-full px-3 py-2 border rounded-lg text-sm focus:ring-indigo-500"
                    >
        ...
        >
            <form @submit.prevent="submitBulkDelete" class="space-y-4">
                <p class="text-sm text-gray-600 mb-4">
                    ...
                    penghapusan (akan dicatat di database).
                </p>
                    ...
                    <div class="flex flex-col gap-2">
                        <label class="flex items-center">
                            <input
                                ...
                                value="2"
                                required
                                class="text-indigo-600 focus:ring-indigo-500 mr-2"
                            />
...
async function fetchSiswa(page = 1) {
    ...
    try {
        const res = await api.get(
            `/admin-sekolah/siswa?page=${page}&cari=${searchQuery.value}&kelas=${encodeURIComponent(filterKelas.value)}`,
        );
