<template>
  <div class="space-y-6">
    <div class="flex items-center justify-between mb-4">
      <div>
        <h2 class="text-xl font-bold text-gray-800">Identitas Sekolah</h2>
        <p class="text-sm text-gray-500">Kelola informasi dasar dan logo instansi Anda.</p>
      </div>
    </div>
    ...
    <BaseCard>
      ...
      <form v-else @submit.prevent="submitForm" class="space-y-4">
        ...
        <div class="mt-6 flex justify-end gap-3 border-t pt-4">
          <BaseButton type="submit" variant="primary" :loading="isSubmitting">Simpan Profil Sekolah</BaseButton>
        </div>
      </form>
    </BaseCard>
  </div>
...
async function fetchSekolah() {
  ...
  try {
    const res = await api.get(`/admin-sekolah/identitas`);
    ...
    toast.error('Gagal mengambil identitas sekolah');
  } finally {
...
async function submitForm() {
  ...
  try {
    ...
    await api.post(`/admin-sekolah/identitas`, formData, {
