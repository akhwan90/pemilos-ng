		...
		<BaseCard class="p-0">
			<div class="p-4 border-b border-gray-200 flex justify-between items-center bg-gray-50 flex-wrap gap-4">
				...
				<div class="flex gap-2 items-center flex-wrap">
					<div class="relative w-48" v-if="auth.user?.level === 2">
						<select v-model="filterTps" @change="fetchDpt(1)" class="w-full pl-3 pr-8 py-2 border rounded-lg text-sm focus:ring-indigo-500 bg-white shadow-sm">
							...
							<option v-for="tps in listTps" :key="tps.kd_kelas" :value="tps.kd_kelas">
								{{ tps.nm_kelas }}
							</option>
						</select>
					</div>
					...
					<BaseButton v-if="selectedIds.length > 0 && auth.user?.level === 2" variant="danger" @click="submitBulkDelete" class="!py-2 text-sm shadow-sm flex gap-1 items-center">
				...
				<template v-else-if="auth.user?.level === 3">
				    <div class="flex gap-2">
    					...
    					<BaseButton variant="danger" @click="cancelToken" class="!py-2 shadow-sm flex items-center gap-2" title="Batal Generate Token">
    						...
    						Batal Generate Token
    					</BaseButton>
    					<BaseButton variant="primary" @click="cetakKartuPemilih" class="!py-2 shadow-sm flex items-center gap-2" title="Batal Generate Token">
				...
				<table class="w-full text-sm text-left">
					...
					<tbody>
						...
						>
							...
							<td class="px-4 py-3 text-center font-bold bg-indigo-50 border-l border-r border-indigo-100 text-indigo-700">
								{{ item.nama_tps }}
							</td>
							<td class="px-4 py-3 text-center font-mono font-bold tracking-widest text-indigo-600 bg-gray-50">
								{{ item.token || '-' }}
							</td>
...
const filteredSiswaBelumDpt = computed(() => {
	...
	return siswaBelumDpt.value.filter((item) => item.nm_siswa.toLowerCase().includes(s) || item.nisn.toLowerCase().includes(s) || item.kelas.toLowerCase().includes(s));
});
...
function cetakKartuPemilih() {
    if (auth.user?.level === 2) {
        window.open('/admin-sekolah/cetak-kartu-pemilih', '_blank');
    } else {
        window.open('/admin-tps/cetak-kartu-pemilih', '_blank');
    }
}
