<template>
    <div class="space-y-6">
        <div class="flex items-center justify-between mb-4">
            <div>
                ...
                <p class="text-sm text-gray-500">
                    Kelola pemetaan siswa yang berhak memilih beserta lokasi
                    TPS-nya.
                </p>
            </div>
        </div>
        ...
        >
            ...
            >
                ...
                <p class="text-3xl font-bold text-green-600">
                    {{ rekapData.sudah_memilih }}
                </p>
            </div>
            <div
                class="bg-white rounded-lg shadow-sm p-4 border-l-4 border-red-500"
            >
                <p class="text-sm text-gray-500 font-medium">Belum Memilih</p>
                <p class="text-3xl font-bold text-red-600">
                    {{ rekapData.belum_memilih }}
                </p>
            </div>
        </div>
        ...
        <BaseCard class="p-0">
            <div class="p-4 border-b border-gray-200 flex justify-between items-center bg-gray-50 flex-wrap gap-4" >
                ...
                <div class="flex gap-2 items-center flex-wrap">
                    <div class="relative w-48">
                        <select
                            v-model="filterTps"
                            @change="fetchDpt(1)"
                            class="w-full pl-3 pr-8 py-2 border rounded-lg text-sm focus:ring-indigo-500 bg-white shadow-sm"
                        >
                            <option value="">-- Semua TPS --</option>
                            <option
                                v-for="tps in listTps"
                                :key="tps.kd_kelas"
                                :value="tps.kd_kelas"
                            >
                                {{ tps.nm_kelas }}
                    ...
                    <div class="relative w-64">
                        <input
                            v-model="searchQuery"
                            @keyup.enter="fetchDpt(1)"
                            type="text"
                            placeholder="Cari NISN atau Nama Siswa..."
                            class="w-full pl-10 pr-4 py-2 border rounded-lg text-sm focus:ring-indigo-500 shadow-sm"
                        />
                        <div
                    ...
                    <BaseButton
                        variant="secondary"
                        @click="fetchDpt(1)"
                        class="!py-2 text-sm shadow-sm"
                        >Cari</BaseButton
                    >

                    <template v-if="auth.user?.level === 3">
                        <label
                            class="flex items-center gap-2 text-sm text-gray-700 ml-4 cursor-pointer"
                        >
                            <input
                                type="checkbox"
                                v-model="filterBelumMemilih"
                                @change="fetchDpt(1)"
                        ...
                        <label
                            class="flex items-center gap-2 text-sm text-gray-700 ml-2 cursor-pointer border-l border-gray-300 pl-4"
                        >
                            <input
                                type="checkbox"
                ...
                <template v-if="auth.user?.level === 2">
                    <BaseButton
                        variant="primary"
                        @click="openAddDptModal"
                        class="!py-2 shadow-sm flex items-center gap-2"
                    >
                        <svg
                            class="w-4 h-4"
                            fill="none"
                ...
                <table class="w-full text-sm text-left">
                    ...
                    >
                        <tr>
                            ...
                            <th
                                class="px-4 py-3 text-center bg-indigo-50 border-l border-r border-indigo-100 text-indigo-800"
                            >
                                TPS Saat Ini
                            </th>
                            ...
                            <th
                                ...
                                class="px-4 py-3 w-20 text-center"
                            >
                                Aksi
                            </th>
                        </tr>
                    ...
                    <tbody>
                        ...
                        >
                            <td
                                ...
                                class="px-4 py-8 text-center text-gray-500"
                            >
                                Data DPT kosong. Silakan tambah DPT baru.
                            </td>
                        </tr>
                        <tr
                            v-else
                        ...
                        >
                            ...
                            <td class="px-4 py-3">
                                {{ item.nama_kelas_asal }}
                            </td>
                            <td
                                class="px-4 py-3 text-center font-bold bg-indigo-50 border-l border-r border-indigo-100 text-indigo-700"
                            >
                                {{ item.nama_tps }}
                            </td>
                            <td
                                class="px-4 py-3 text-center font-mono font-bold tracking-widest text-indigo-600 bg-gray-50"
                            >
                                {{ item.token || "-" }}
                            </td>
                            <td class="px-4 py-3 text-center">
                                <span
                                    v-if="item.pilihan"
                                    class="px-2 py-1 text-xs font-semibold rounded bg-green-100 text-green-800 flex items-center justify-center gap-1"
                                >
                                    <svg
                                        class="w-3 h-3"
                                        fill="none"
                                ...
                                <span
                                    v-else
                                    class="px-2 py-1 text-xs font-semibold rounded bg-gray-100 text-gray-600"
                                    >Belum</span
                                >
                            </td>
                            <td
                            ...
                            >
                                <button
                                    ...
                                    "
                                    class="text-red-600 hover:text-red-900 bg-red-50 px-2 py-1 rounded text-xs"
                                >
                                    Keluarkan
                                </button>
                            </td>
            ...
            <!-- Pagination Footer -->
            <div
                class="p-4 border-t flex justify-between items-center text-sm text-gray-600 bg-white"
            >
                <div>
                ...
                <div class="flex gap-2">
                    <button
                        @click="fetchDpt(pagination.current_page - 1)"
                    ...
                    <span
                        class="px-3 py-1 font-semibold text-gray-800 border rounded bg-gray-50"
        ...
        <BaseModal
            ...
            title="Tambah Siswa ke DPT"
            max-width="5xl"
        >
            <div class="flex gap-4">
                <!-- Panel Kiri: List Siswa Belum DPT -->
                <div
                    class="w-3/5 border rounded-lg overflow-hidden flex flex-col h-[600px]"
                >
                    ...
                    >
                        <h4 class="font-bold text-gray-700 text-sm">
                            Daftar Siswa Belum DPT ({{
                                filteredSiswaBelumDpt.length
                            }})
                        </h4>
                        <input
                            v-model="searchSiswaBelumDpt"
                            type="text"
                            placeholder="Cari nama nisn kelas..."
                            class="px-3 py-1 text-sm border rounded w-48 focus:ring-indigo-500"
                        />
                    </div>
                    ...
                    <div class="overflow-y-auto flex-1 bg-white">
                        <table class="w-full text-sm text-left relative">
                            <thead
                                class="text-xs text-gray-700 uppercase bg-gray-50 border-b sticky top-0 z-10 shadow-sm"
                            >
                                <tr>
                                    <th class="px-3 py-2 w-10 text-center">
                                        <input
                                            type="checkbox"
                                            @change="toggleAllSiswaModal"
                                            :checked="isAllSiswaModalSelected"
                            ...
                            <tbody>
                                ...
                                <tr
                                    v-else-if="
                                        filteredSiswaBelumDpt.length === 0
                                    "
                                    class="border-b"
                                >
                                    <td
                                        colspan="4"
                                        class="px-3 py-6 text-center text-gray-500 bg-yellow-50"
                                    >
                                        Semua siswa aktif sudah masuk ke DPT.
                                    </td>
                                </tr>
                                <tr
                                    v-else
                                    v-for="siswa in filteredSiswaBelumDpt"
                                    :key="siswa.nisn"
                                    class="border-b hover:bg-indigo-50 cursor-pointer"
                                    @click="toggleRowSelection(siswa.nisn)"
                                >
                                    <td
                ...
                <div class="w-2/5 flex flex-col gap-4">
                    <BaseCard class="bg-indigo-50 border-indigo-200">
                        <h4
                            class="font-bold text-indigo-800 mb-2 border-b border-indigo-200 pb-2"
                        >
                            Target Alokasi TPS
                        </h4>
                        <p class="text-xs text-indigo-600 mb-4">
                            Pilih ke TPS / Kelas mana siswa yang dicentang akan
                        ...
                        <div class="bg-white p-4 rounded border text-center">
                            <span
                                class="block text-3xl font-black text-indigo-600 mb-1"
                                >{{ selectedNisnToDpt.length }}</span
                            >
                            <span
                        ...
                        <div class="mt-6 flex justify-end gap-2">
                            ...
                            <BaseButton
                                type="button"
                                variant="primary"
                                @click="submitBulkInsert"
                                :loading="isSubmitting"
                                :disabled="
                                    selectedNisnToDpt.length === 0 ||
                                    !targetTpsId
                                "
                                class="w-2/3"
                                >Simpan ke DPT</BaseButton
                            >
                        </div>
                    </BaseCard>
                </div>
...
const filteredSiswaBelumDpt = computed(() => {
    ...
    return siswaBelumDpt.value.filter(
        (item) =>
...
const isAllSiswaModalSelected = computed(() => {
    ...
    return (
        visible.length > 0 &&
        visible.every((s) => selectedNisnToDpt.value.includes(s.nisn))
    );
});
...
const generateToken = async () => {
    if (
        !confirm(
            "Apakah Anda yakin ingin meng-generate token baru untuk seluruh DPT di TPS Anda?\n\nToken lama (jika ada) akan hangus/diganti.",
        )
    ) {
        return;
    }
    ...
    try {
        ...
        toast.success(
            res.data.message || "Token berhasil di-generate",
            "Sukses",
        );
        fetchDpt(pagination.value.current_page);
    } catch (error) {
        toast.error(
            error.response?.data?.message || "Gagal melakukan generate token",
            "Error",
        );
    }
};
...
const cancelToken = async () => {
    if (
        !confirm(
            "Yakin ingin membatalkan/menghapus token untuk SELURUH DPT di TPS ini?",
        )
    ) {
        return;
    }
    ...
    try {
        ...
        toast.success(
            res.data.message || "Token berhasil dibatalkan",
            "Sukses",
        );
        fetchDpt(pagination.value.current_page);
    } catch (error) {
        toast.error(
            error.response?.data?.message || "Gagal membatalkan token",
            "Error",
        );
    }
};
...
async function fetchDpt(page = 1, showLoading = true) {
    ...
    try {
        const res = await api.get(
            `/admin-sekolah/dpt?page=${page}&cari=${searchQuery.value}&tps_id=${filterTps.value}&belum_memilih=${filterBelumMemilih.value}`,
        );
...
function toggleAllSiswaModal(event) {
    ...
    if (event.target.checked) {
        selectedNisnToDpt.value = [
            ...new Set([...selectedNisnToDpt.value, ...visibleNisn]),
        ];
    } else {
        selectedNisnToDpt.value = selectedNisnToDpt.value.filter(
            (nisn) => !visibleNisn.includes(nisn),
        );
    }
}
...
async function submitBulkInsert() {
    ...
    try {
        ...
        toast.error(e.response?.data?.message || "Terjadi kesalahan saat memasukkan data ke DPT");
    } finally {
...
async function submitBulkDelete() {
    ...
    if (
        !confirm(
            `Yakin ingin mengeluarkan ${selectedIds.value.length} siswa ini dari DPT?\n(Mereka akan kembali ke daftar "Belum DPT")`,
        )
    )
        return;
...
async function submitSingleDelete(id, isSudahMemilih, token) {
    if (isSudahMemilih) {
        return toast.error(
            "Siswa ini sudah menggunakan hak pilihnya (Sudah Mencoblos). Tidak bisa dikeluarkan dari DPT!",
        );
    }
    if (token) {
        return toast.error(
            "Siswa ini sudah memiliki token pemilihan. Silakan hubungi TPS terkait atau reset token terlebih dahulu.",
        );
    }
