  ...
  <div class="min-h-screen bg-gradient-to-br from-blue-50 via-white to-indigo-50">
    <header class="bg-white border-b border-gray-200 shadow-sm">
      <div class="max-w-4xl mx-auto px-4 py-3 flex items-center gap-3">
        ...
        <div class="w-8 h-8 bg-emerald-600 rounded-lg flex items-center justify-center"><span class="text-white font-bold text-sm">D</span></div>
        <div><h1 class="text-sm font-semibold text-gray-800">Tamu DPRD</h1><p class="text-xs text-gray-500">GESIT</p></div>
      </div>
    </header>
    <div class="max-w-3xl mx-auto px-4 py-8">
      <BaseCard>
        <template #header>
          <h2 class="text-xl font-bold text-gray-800 mb-2">Pendaftaran Tamu DPRD</h2>
          <p class="text-sm text-gray-500">Daftarkan kunjungan kerja spesifik kepada Alat Kelengkapan Dewan (Alkap) di DPRD.</p>
        </template>
        ...
        <form @submit.prevent="handlePreSubmit">
          <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
            ...
            <BaseInput v-model="form.nama_alkap" label="Alat Kelengkapan Dewan (Alkap)" required />
          ...
          <div class="mt-8 pt-6 border-t border-gray-200">
            ...
            <BaseButton type="submit" variant="success" :disabled="!captchaPassed" :loading="loading" :block="true" class="md:w-auto px-8 py-3">
              Kirim Formulir
            </BaseButton>
          </div>
        </form>
...
const hariList = ['Senin', 'Selasa', 'Rabu', 'Kamis', "Jum'at", 'Sabtu'];
const alkapList = ['Komisi I', 'Komisi II', 'Komisi III', 'Komisi IV', 'Badan Anggaran', 'Badan Pembentukan Perda', 'Pimpinan Dewan'];

// Map simple arrays to object options expected by BaseSelect
const hariOptions = hariList.map(h => ({ value: h, label: h }));
const alkapOptions = alkapList.map(a => ({ value: a, label: a }));
...
const form = reactive({
  nama: '', instansi: '', hari_berkunjung: '', jam_berkunjung: '', tanggal_berkunjung: '',
  nama_alkap: '', jumlah_peserta: 1, nama_jabatan_ketua_rombongan: '',
...
async function submitForm() {
  ...
  try {
    await api.post('/tamu-dprd', fd, { headers: { 'Content-Type': 'multipart/form-data' } });
    showConsent.value = false;
    router.push({ path: '/sukses', query: { type: 'dprd' } });
  } catch (e) {
    showConsent.value = false;
