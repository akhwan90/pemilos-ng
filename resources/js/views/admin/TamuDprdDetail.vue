<template>
  <div>
    <div class="flex items-center gap-3 mb-6">
      <router-link to="/admin/tamu-dprd" class="text-gray-400 hover:text-gray-600">
        <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M15 19l-7-7 7-7" /></svg>
      </router-link>
      <h1 class="text-2xl font-bold text-gray-800">Detail Tamu DPRD</h1>
    </div>
        ...
        <div class="lg:col-span-2 space-y-6">
          <div class="bg-white rounded-xl shadow-sm border border-gray-200 p-6">
            ...
            <dl class="grid grid-cols-2 gap-4">
              ...
              <div><dt class="text-xs text-gray-500">Hari</dt><dd class="text-sm font-medium text-gray-800">{{ item.hari_berkunjung }}</dd></div>
              <div><dt class="text-xs text-gray-500">Jam</dt><dd class="text-sm font-medium text-gray-800">{{ item.jam_berkunjung }}</dd></div>
              <div><dt class="text-xs text-gray-500">Tanggal</dt><dd class="text-sm font-medium text-gray-800">{{ item.tanggal_berkunjung }}</dd></div>
              <div><dt class="text-xs text-gray-500">Alkap Tujuan</dt><dd class="text-sm font-medium text-gray-800">{{ item.nama_alkap }}</dd></div>
          ...
          <div class="bg-white rounded-xl shadow-sm border border-gray-200 p-6">
            ...
            <div class="space-y-2">
              <a v-if="item.file_surat_kunjungan" :href="item.file_surat_kunjungan" target="_blank" class="block px-4 py-2 bg-emerald-50 text-emerald-700 rounded-lg text-sm font-medium hover:bg-emerald-100">Download Surat Kunjungan</a>
              <a v-if="item.file_spt" :href="item.file_spt" target="_blank" class="block px-4 py-2 bg-emerald-50 text-emerald-700 rounded-lg text-sm font-medium hover:bg-emerald-100">Download SPT</a>
              <a v-if="item.file_bukti_menginap" :href="item.file_bukti_menginap" target="_blank" class="block px-4 py-2 bg-emerald-50 text-emerald-700 rounded-lg text-sm font-medium hover:bg-emerald-100">Download Bukti Menginap</a>
            </div>
          </div>
...
const whatsappUrl = computed(() => {
  ...
  const msg = encodeURIComponent(`Yth. ${item.value.nama},\n\nKami dari Sekretariat DPRD Kab. Kulon Progo menindaklanjuti pendaftaran kunjungan ke DPRD melalui sistem GESIT.\n\nMohon kesediaannya untuk konfirmasi jadwal.\n\nTerima kasih.`);
  return `https://wa.me/${phone?.startsWith('62') ? '' : '62'}${phone}?text=${msg}`;
});

onMounted(async () => {
  try { const res = await api.get(`/admin/tamu-dprd/${route.params.id}`); item.value = res.data.data; newStatus.value = item.value.status; } catch (e) { console.error(e); } finally { loading.value = false; }
});

async function updateStatus() {
  try { const res = await api.patch(`/admin/tamu-dprd/${route.params.id}/status`, { status: newStatus.value }); item.value = res.data.data; } catch (e) { console.error(e); }
}

async function addNote() {
  if (!newNote.value.trim()) return;
  try { const res = await api.post(`/admin/notes/dprd/${route.params.id}`, { note: newNote.value }); item.value.notes.push(res.data.data); newNote.value = ''; } catch (e) { console.error(e); }
}
</script>
