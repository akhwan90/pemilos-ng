  ...
  <div>
    <div class="flex items-center justify-between mb-6">
      <div class="flex items-center gap-3">
        <button @click="$router.push('/admin/tamu-setwan')" class="text-gray-500 hover:text-gray-700">
          <svg class="w-6 h-6" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M10 19l-7-7m0 0l7-7m-7 7h18" /></svg>
        </button>
        <h2 class="text-xl font-bold text-gray-800">Statistik Tamu Setwan</h2>
        <p class="text-sm text-gray-500">Ringkasan data kunjungan Tamu Setwan tahun ini.</p>
      </div>
...
async function loadStats() {
  loading.value = true;
  try {
    const res = await api.get('/admin/tamu-setwan/stats', {
