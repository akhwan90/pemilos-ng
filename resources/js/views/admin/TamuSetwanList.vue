<template>
  <div>
    <div class="flex items-center justify-between mb-6">
      <h1 class="text-2xl font-bold text-gray-800">Tamu Setwan</h1>
      <div class="flex items-center gap-2">
        ...
        <BaseButton @click="$router.push('/admin/tamu-setwan/stats')" variant="secondary">
      ...
      <BaseCard v-for="(item, i) in items" :key="item.id" class="hover:shadow-md transition-shadow">
        <div class="flex flex-col md:flex-row gap-6">
          ...
          <div class="md:w-2/4 border-t md:border-t-0 md:border-l border-gray-100 pt-4 md:pt-0 md:pl-6 flex-1 flex flex-col">
            <div class="mb-2">
              <p class="text-sm font-bold text-gray-800">{{ item.instansi }}</p>
            </div>
    ...
    <BaseModal v-model="showFormModal" :title="isEdit ? 'Edit Tamu Setwan' : 'Tambah Tamu Setwan'" maxWidth="4xl">
    ...
    <!-- Detail Modal -->
    <BaseModal v-model="showDetailModal" title="Detail Kunjungan Tamu Setwan" maxWidth="4xl">
...
const whatsappUrl = computed(() => {
  ...
  const msg = encodeURIComponent(
    `Yth. ${selectedItem.value.nama} (${selectedItem.value.instansi}),\n\nKami dari Sekretariat DPRD Kab. Kulon Progo menindaklanjuti rencana kunjungan Anda pada ${selectedItem.value.tanggal_berkunjung}.\n\nMohon kesediaannya untuk dihubungi lebih lanjut.\n\nTerima kasih.`
  );
...
async function submitForm() {
  ...
  try {
    ...
    if (isEdit.value) {
      ...
      await api.post(`/admin/tamu-setwan/${selectedItem.value.id}`, formData, {
        headers: { 'Content-Type': 'multipart/form-data' }
      });
    } else {
      await api.post('/admin/tamu-setwan', formData, {
        headers: { 'Content-Type': 'multipart/form-data' }
      });
    }
...
async function deleteItem(id) {
  ...
  try {
    await api.delete(`/admin/tamu-setwan/${id}`);
    loadData();
  } catch (e) {
    console.error(e);
...
async function generateDocument() {
  ...
  try {
    const res = await api.post(`/admin/tamu-setwan/${selectedItem.value.id}/generate`, {
...
async function submitUpload() {
  ...
  try {
    ...
    const res = await api.post(`/admin/tamu-setwan/${selectedItem.value.id}/upload-berkas`, formData, {
      headers: { 'Content-Type': 'multipart/form-data' }
    });
...
async function hapusBerkas() {
  ...
  try {
    const res = await api.delete(`/admin/tamu-setwan/${selectedItem.value.id}/hapus-berkas`, {
...
async function loadData() {
  try {
    ...
    const res = await api.get('/admin/tamu-setwan', { params });
...
async function exportData() {
  try {
    const params = {};
    if (filterStatus.value) params.status = filterStatus.value;
    const res = await api.get('/admin/tamu-setwan/export', { params });
    const blob = new Blob([JSON.stringify(res.data.data, null, 2)], { type: 'application/json' });
    const a = document.createElement('a');
    a.href = URL.createObjectURL(blob);
    a.download = `tamu-setwan-export.json`;
    a.click();
